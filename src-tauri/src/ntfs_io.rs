...
/// 打开 NTFS 卷句柄（辅助函数）
#[cfg(windows)]
fn open_volume_handle(drive: &str, admin_hint: bool) -> Result<windows::Win32::Foundation::HANDLE, String> {
    ...
    unsafe {
        ...
        CreateFileW(
            ...
            None,
        ).map_err(|e| format!("Cannot open volume: {:?}. {}.", e, hint))
    }
}
...
/// 通过读取卷的第一个扇区来获取参数
#[cfg(windows)]
pub fn get_volume_info_from_boot_sector(drive: &str) -> Result<NtfsVolumeInfo, String> {
    use windows::Win32::Storage::FileSystem::*;
    use windows::Win32::Foundation::*;
    ...
    unsafe {
        ...
        if ReadFile(handle, Some(&mut boot_sector), Some(&mut bytes_read), None).is_err() {
            let _ = CloseHandle(handle);
            return Err("Unable to read the boot sector".to_string());
        }
...
#[cfg(windows)]
pub fn read_mft_raw(drive: &str, vol_info: &NtfsVolumeInfo) -> Result<Vec<u8>, String> {
    use windows::Win32::Storage::FileSystem::*;
    use windows::Win32::Foundation::*;
    ...
    unsafe {
        ...
        if SetFilePointerEx(handle, aligned_offset as i64, Some(&mut new_pos), FILE_BEGIN).is_err() {
            let _ = CloseHandle(handle);
            return Err(format!("Unable to locate MFT record 0 (alignment offset: {})", aligned_offset));
        }
        ...
        if ReadFile(handle, Some(&mut read_buffer), Some(&mut bytes_read), None).is_err() {
            let _ = CloseHandle(handle);
            return Err("Unable to read MFT record 0".to_string());
        }
        ...
        let first_record = if offset_within_sector as usize + record_size <= read_buffer.len() {
            ...
            let _ = CloseHandle(handle);
            return Err("Insufficient data read".to_string());
        };
...
#[cfg(windows)]
pub fn get_mft_size_from_record0(record_bytes: &[u8]) -> Result<u64, String> {
...
#[cfg(windows)]
pub fn parse_data_runs_from_record0(record_bytes: &[u8]) -> Result<Vec<DataRun>, String> {
    if record_bytes.len() < 42 || &record_bytes[0..4] != b"FILE" {
        return Err("无效的 MFT 记录".to_string());
    }
    ...
    while attr_offset + 8 < record_bytes.len() {
        ...
        if attr_type == 0x80 && record_bytes[attr_offset + 8] != 0 {
            ...
            return parse_data_runs_bytes(&record_bytes[runs_start..]);
        }
...
/// 解析 Data Runs 字节数组
#[cfg(windows)]
pub fn parse_data_runs_bytes(data: &[u8]) -> Result<Vec<DataRun>, String> {
